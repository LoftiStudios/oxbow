...
struct ResumeEndToEndTests {
  ...
  /// The same public VOD `docs/design/resume.md` and
  /// `docs/design/fragmented-output.md` were spiked against.
  private static let videoID = "1480816483"
  ...
  @Test func aHardKilledCompositeResumesAndDeliversTheCorrectFile() async throws {
    ...
    // is downloaded. `qualities.first` is what an empty (default) quality
    // resolves to for a composite job (compositing.md §4).
    let info = try await VideoInfoFetcher.fetch(id: Self.videoID, helper: helper, process: HelperProcess())
    let quality = try #require(info.qualities.first, "VOD has no qualities to composite")
    ...
    let chatRequest = ChatRequest(
      videoID: Self.videoID, trimStart: Self.trimStart, trimEnd: Self.trimEnd, format: .json)
    ...
    try await run(.downloadVideo(videoRequest), context: video1Context, executable: helper)
    ...
    try await run(.downloadChat(chatRequest), context: chat1Context, executable: helper)
    ...
    try await run(.renderChat(renderRequest), context: render1Context, executable: helper)
    ...
    try await run(.downloadVideo(videoRequest), context: video2Context, executable: helper)
    ...
    try await run(.downloadChat(chatRequest), context: chat2Context, executable: helper)
    ...
    try await run(.renderChat(renderRequest), context: render2Context, executable: helper)
    ...
    // MARK: Audio: completeness and sync, proven against a valid (non-corrupted) audio track
    //
    // The real sidecar is corrupted by the hard kill above (see the finding
    // below), so it cannot itself be used to check "does the delivered
    // audio run to the end and stay in sync". This checks the concat +
    // audio-mapping mechanism `.assemble` uses with a *freshly extracted*,
    // valid copy of the same audio — proving the mechanism is sound and
    // isolating the defect to the sidecar's own crash-survivability, not to
    // assemble's approach. Extracted from `video2` before assemble's
    // `makeContext` call deletes it below.
    ...
    // MARK: THE FINDING — a hard kill during the first attempt corrupts the audio sidecar
    ...
    // `.composite` case, `sidecar`. Only the piece output carries
    // `-movflags +frag_keyframe+empty_moov+…`; the sidecar does not. A
    // conventional MP4's index is written once, at the very end, so killing
    // the *same process* that is still writing it — which is exactly what
    // just happened above, and exactly the scenario resume exists for —
    // leaves it with no `moov` at all: unreadable, and never rewritten by
    // any later attempt (the sidecar is only ever written on the first).
    let audioSidecar = workspace.resumeDirectory(jobID).appending(path: "audio.m4a")
    let audioSize = try FileManager.default.attributesOfItem(atPath: audioSidecar.path)[.size] as? Int ?? -1
    let audioBanner = try await streamBanner(of: audioSidecar, ffmpeg: ffmpeg)
    let audioIsReadable = audioBanner.contains("Stream #")
    print("Audio sidecar after the hard kill: \(audioSize) bytes, "
      + "readable=\(audioIsReadable) — \(audioBanner.contains("moov atom not found") ? "no moov atom" : "")")

    // MARK: The real `.assemble` step, run exactly as `QueueEngine` would
    ...
    if case .exited(0) = assembleResult.status {
      print("`.assemble` unexpectedly succeeded — the audio sidecar corruption above did not "
        + "reproduce on this run. Re-check the finding before trusting this line.")
    } else {
      print("`.assemble` failed, as the corrupted sidecar predicts:\n\(assembleResult.stderr.suffix(500))")
    }

    #expect(
      audioIsReadable == false,
      Comment(rawValue: "REGRESSION CHECK, not a desired outcome: the audio sidecar is expected "
        + "to be unreadable after a hard kill on the first attempt (see the finding above). If "
        + "this now fails, either FFmpeg's behaviour changed or someone fixed the sidecar's own "
        + "container — re-verify docs/design/resume.md before updating this assertion."))
    ...
    print("""
      ...
      Delivered (video-only concat): \(deliveredFrames) frames
      ...
      Audio sidecar after hard kill: \(audioSize)B, readable=\(audioIsReadable)
